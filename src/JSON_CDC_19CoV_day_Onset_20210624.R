rm(list=ls())
# JSON https://data.cdc.gov.tw/dataset/aagstable-day-19cov

url <- "https://od.cdc.gov.tw/eic/Age_County_Gender_day_19Cov.json"
library(jsonlite)
t <- fromJSON(url)
library(readr)
result_ori <- read_csv("data/CDC_19CoV_day_Onset_temp_Big5.csv", 
...
t_diff <- merge(t2,t3,by=c("確定病名","縣市","鄉鎮","性別","是否為境外移入","年齡層","發病日"),all = TRUE)
...
savef <- function(time){
  write.csv(result,paste0("./data/CDC_19CoV_day_Onset_",time,"_Big5",".csv"),row.names = FALSE);
  write.csv(result,paste0("./data/CDC_19CoV_day_Onset_","temp","_Big5",".csv"),row.names = FALSE);
  write.csv(result_op.dt,paste0("./data/CDC_19CoV_day_Onset_CountyTown_",time,"_Big5",".csv"),row.names = FALSE);
  write.csv(t_diff,paste0("./data/CDC_19CoV_day_Onset_diff_",time,"_Big5",".csv"),row.names = FALSE);
  write.csv(Y.diff,paste0("./data/CDC_19CoV_day_Onset_diff_town_",time,"_Big5",".csv"),row.names = FALSE);
